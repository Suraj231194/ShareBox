...
const uploadFiles = async (req, res) => {
  ...
  try {
    ...
    fs.appendFileSync('server_error.log', `${new Date().toISOString()} - Upload Error: ${error.stack}\n`);
...
const getUserFiles = async (req, res) => {
  const { userId } = req.params;
  try {
    const files = await File.find({ createdBy: userId }).sort({ createdAt: -1 }); // Sort by newest first

    if (!files.length) {
      return res.status(200).json([]); // Return empty array if no files
    }
    ...
    const filesWithSignedUrls = await Promise.all(
      files.map(async (file) => {
        try {
          ...
          return {
            ...file.toObject(),
            path: signedUrl, // Replace permanent private URL with temporary public signed URL
          };
        } catch (err) {
          console.error(`Error generating signed URL for file ${file._id}:`, err);
          return file; // Fallback to original if signing fails
        }
      })
    );

    return res.status(200).json(filesWithSignedUrls);
